...
Which shape a collision takes depends on the argument count (`overlay(a, b, 1, 2)` parses as the builtin, `overlay(a)` is a `PARSE_ERROR`), which is exactly why the census tests several arities rather than reasoning about the grammar. Four of the eleven — `overlay`, `pad`, `normalize`, `reverse` — are stdlib function names too, and an earlier version of this document claimed only `pad` was hurt by that. Measured since: **three of the four stdlib entries are unreachable by their bare names.** `pad(f, w, h)` is a `PARSE_ERROR` outright ("Required keyword: 'is_left' missing") and the 4-arg form arrives argless; `normalize(a, -23)` is a `PARSE_ERROR` and `normalize(a)` arrives argless; `reverse(f)` arrives argless too. All three entries sit in the function table but cannot be called by name. Only `overlay` survives, because its 4-argument shape happens to map onto the builtin grammar and lowering un-parks the arguments. The workaround for the other three is the namespace with the filter's own option names — `ffmpeg.pad(...)`, `ffmpeg.loudnorm(...)` (which is what stdlib `normalize` wraps anyway), `ffmpeg.reverse(...)` — losing the stdlib's hand-picked ergonomics; renaming the three stdlib entries is under consideration.
