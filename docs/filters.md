...
With no ffmpeg on `PATH` at all, sqlmpeg is a parser: SQL structure, aliases, trims, sinks and remaps all still check out and errors stay line-anchored, but any filter call is rejected with a typed "ffmpeg not found" rather than a guess about what your machine might someday have. A query compiles against the ffmpeg that will run it - that is the honest contract, and `sqlmpeg explain` will show you exactly what the registry resolved.
