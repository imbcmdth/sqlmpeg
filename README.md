...
Or run it without installing anything: `uvx sqlmpeg` / `pipx run sqlmpeg`. Python 3.10+, one dependency (`sqlglot`). ffmpeg is not required to compile - the stdlib works anywhere - but having `ffmpeg`/`ffprobe` on `PATH` unlocks the good stuff: probing your files, calling every filter your build ships, and actually running the result.
...
|---|---|---|
| `compile` | print the full ffmpeg command | `--graph-only` (just the filtergraph string) · `-o PATH` (output path; default is the query's `COPY` sink path, else `out.mp4`) · `--no-probe` · `--portable` |
| `explain` | dump the compiled IR graph as JSON | `--no-probe` · `--portable` |
| `validate` | exit 0 if the query compiles, else a line-anchored error | `--json` (machine-readable error object on stdout) · `--no-probe` · `--portable` |
| `run` | compile, then execute ffmpeg | `-o PATH` (required unless the query carries a sink) · `--timeout SECS` (default 600) · `-y` (overwrite) |
| `prompt` | print the LLM system prompt | `--dynamic` (append every filter *your* ffmpeg reports) |

The two compile-mode flags, since they come up everywhere:

- `--no-probe` - skip ffprobe entirely for a byte-reproducible, fully offline compile. Bounds checks and provenance metadata go away; `SELECT *` and bare-array splats (which need a readable file to size) are rejected.
- `--portable` - compile against the stdlib alone, exactly what a machine with no ffmpeg installed would see. Run it before handing a query to someone else's machine.
...
- **Bare arrays broadcast.** `reverb(v.audio, 0.3)` fans out one node per track, each output keeping its language tag. Two arrays in one call zip elementwise.
...
- **Two filter tiers.** The stdlib's few dozen functions compile anywhere with hand-picked signatures; the other ~450 filters in your ffmpeg build are callable by name with options as named arguments (`unsharp(a.frame, luma_amount => 1.5)`), type-checked against what the binary reports. `ffmpeg.<name>(...)` always means the raw filter, including the eleven names Postgres grammar would otherwise eat. A few multi-output filters (`channelsplit`, `acrossover`, `extractplanes`) return arrays. [docs/dynamic-filters.md](docs/dynamic-filters.md) has the whole story.
...
- **`enable` and expressions.** `blur(a.frame, 12, enable => 'between(t,10,20)')` windows an effect in time; expression strings like `'(W-w)/2'` do per-frame geometry in slots that accept them.
...
Docs: [cookbook](docs/examples.md) · [dynamic filters](docs/dynamic-filters.md) · [trimming](docs/trimming.md) · [error contract](docs/errors.md) · [system prompt](docs/system-prompt.md) · [project spec](sqlmpeg-project.md)
