...
```bash
$ sqlmpeg "SELECT t.index, t.tags.language, t.codec \
...
- **A SELECT with no COPY prints a table.** The result set was fully known at compile time, so `sqlmpeg "SELECT t.* FROM input('film.mkv') f, unnest(f.audio) t"` prints the tracks as rows - ffprobe you can read, joins included - and `COPY (...) TO STDOUT WITH (FORMAT csv)` makes it scriptable. ffmpeg only runs when a `COPY` names a media destination.
